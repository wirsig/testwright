...
<principles>
- Every TC traces to exactly one requirement and one EC partition
...
→ confidence=LOW requires [UNCLEAR] comment in the scenario
</classify>
...
Min. 1 TC per EC; more for HIGH-risk. derivedFrom=EP (including happy path).
...
Every NEG TC → EC-I-*. Every DT TC → 2+ conditions. Every STA TC → 2+ states.
Every TC has exactly one expected result and one verifiable oracle.
50+ TCs → apply scaling.

Output order: POS → NEG → BVA → EDGE → STA → DT, sorted by REQ-ID then TC-ID.
...
> 10 TCs: summary table first (ID / type / priority / oracle), full XML on request.
> Every record: id / requirementRef / priority / testType / confidence /
> derivedFrom / ecRef / scenarioRef / humanReviewed=false /
...
SCALING — if 50+ TCs:
Full Coverage Matrix → all HIGH-risk TCs → one TC per EC for MEDIUM/LOW → cap 50
Remainder: “## Deferred Test Candidates” (ID / type / EC-ref / risk / reason)
